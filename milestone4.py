"""
Combined Graph Analytics + Semantic QA with Contextual Cross-Domain Linking
- Graph Analytics: centralities, communities, PyVis graph
- Semantic QA: semantic search using sentence embeddings, node-level PyVis graph
- Shows category legend only for the result entity's domain

Run:
streamlit run combined_graph_cross_domain_app.py
"""
...
if st.button("Search QA"):
    if not question.strip():
        ...
        if not results:
            ...
            if entity_cat:
                st.write("### Category for This Result")
